...
function contentTypeFor(ext) {
  switch (ext) {
    case '.c': return 'text/plain; charset=utf-8';
...
module.exports = (req, res) => {
  ...
  const base = path.join(process.cwd(), 'c');
  ...
  try {
    ...
    if (fs.existsSync(full) && fs.statSync(full).isFile()) {
      ...
      res.setHeader('x-served-by', 'c-api-local');
      res.statusCode = 200;
    ...
    try {
      ...
      if (candidate) {
        ...
        if (fs.existsSync(matchedFull) && fs.statSync(matchedFull).isFile()) {
          ...
          res.setHeader('x-served-by', 'c-api-case-insensitive-match');
          res.statusCode = 200;
    ...
    // Fallback to GitHub raw (repo path uses `c` folder)
    const owner = 'milan-m-antony';
    const repo = 'code-vault';
    const branch = 'main';
    const https = require('https');
    const rawUrl = `https://raw.githubusercontent.com/${owner}/${repo}/${branch}/c/${rel}`;
